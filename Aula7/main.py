...
from leitor import LeitorDeArquivos
...
"""try:
    leitor = LeitorDeArquivos("arquivo.txt")
    ...
    print("Exceção do tipó IOError capturada e tratada")
finally:
    if 'leitor' in locals():
        leitor.fechar()"""

with LeitorDeArquivos('arquivo.txt') as leitor:
    leitor.ler_proxima_linha()
    leitor.ler_proxima_linha()
    leitor.ler_proxima_linha()
    leitor.ler_proxima_linha()